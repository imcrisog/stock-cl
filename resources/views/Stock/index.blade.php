...
<div class="relative m-0 my-2 mb-4 md:mb-4 md:m-4 w-full h-full flex bg-slate-800 rounded-md flex-col z-[1] overflow-auto scroll-smooth " style="scrollbar-width: thin;">
    <div class="w-full md:w-[95%] my-1 md:my-2 md:m-4 mb-0 flex flex-row justify-center md:justify-between items-center absolute">
        ...
        <section class="flex flex-row gap-x-2 items-center w-full md:w-1/2 justify-center ">
            @if (!empty($findStock))
            ...
            <div class="flex-inline items-center text-gray-900 font-medium w-full">
                    <form action="{{route('stocks.index', request()->search);}}" class="flex md:flex gap-x-2" method="post" id="searchForm">
                        <div class=" [&>*]:md:py-2 [&>*]:md:px-4 gap-x-2 w-fit ">
                            <input type="text" class=" bg-inherit text-white font-semibold border-[1px] border-zinc-500 rounded-md outline-none focus:border-sky-500 ">
                        </div>
                        <button type="submit" class="px-2 py-2 border-2 rounded-lg border-rose-500 text-gray-300 font-extrabold transform duration-200 hover:scale-95 active:scale-75 active:bg-rose-400">BUSCAR</button>
                    </form>
            </div>
        </section>
        @if ($role->id <= 2)
        <div class="w-auto mx-2 flex">
            <a href="{{route('stocks.create')}}" class="p-[0.25rem] w-auto md:max-w-32 font-semibold bg-amber-500 hover:bg-amber-600 rounded-md md:px-4 md:py-2">
                Añadir stock
            </a>
        </div>
        @endif
    ...
    <table class="items-center text-center m-0 md:m-2 mt-20 md:mt-16 space-2 max-h-1/2" id="table">
        <thead class="bg-slate-700 border-t-2 border-b-2 border-gray-500 h-[10%]">
            <tr>
                @foreach($stocksColumns as $key => $value)
                    <th class=" w-fit px-4 font-semibold border-r-[1px] border-gray-400">{{ $value }}</th>
                @endforeach
                @if ($role->id === 1)
                ...
                <tr class="h-4 border-b-[1px] border-slate-500" id="tr">
                    @foreach($stocksColumns as $key => $value)
                        @if (in_array($value, $pricesColumns))
                            <td class="w-fit text-nowrap font-semibold py-2">$ {{ $stock->$value }}</td>
                        @else
                            <td class="w-fit text-nowrap font-semibold py-2">{{ $stock->$value }}</td>
                        @endif
        ...
        @else
            ...
            <tr class="h-[8%] md:h-[1rem] border-b-[1px] border-slate-500" id="tr">
                @foreach($stocksColumns as $key => $value)
                    @if (in_array($value, $pricesColumns))
                        <td class="w-fit text-nowrap font-semibold py-2 hover:bg-slate-600">$ {{ $stock->$value }}</td>
                    @else
                        <td class="w-fit text-nowrap font-semibold py-2 hover:bg-slate-600">{{ $stock->$value }}</td>
                    @endif
                @endforeach
...
<script>
    ...
    searchForm.addEventListener('submit', function(e) {
        e.preventDefault();
        window.location.href = window.location.href.split('?')[0] + '?search=' + searchForm.querySelector('input').value;
    });
